...
contract Staking is Ownable, ReentrancyGuard {
    ...
    mapping(address wallet =>  StakeInfo) public stakeInfo;
    ...
    modifier isStakeExist(address _user) {
        bool isExist = stakeInfo[_user].amount != 0;

        require(isExist, "You don't have stake with this stake");
        _;
    }
    ...
    function unstake() external nonReentrant isStakeExist(msg.sender) {
        uint256 penaltyAmount;
        uint256 amount = stakeInfo[msg.sender].amount;
        uint256 _finishDate = stakeInfo[msg.sender].finishPeriod;
        ...
        stakeInfo[msg.sender] = StakeInfo(0, 0, 0, 0, 0);
        totalStaked = totalStaked - (amount + penaltyAmount);
        stakeCount--;

        emit Unstake(msg.sender, amount + penaltyAmount);
    }

    function claimReward() public nonReentrant isStakeExist(msg.sender) {
        _claimReward(msg.sender);
    }

    function _claimReward(address _user) private {
        uint256 rewards = calculateReward(_user);
        require(amountReward > rewards, "amount reward empty!");
        amountReward -= rewards;
        rewardToken.transfer(_user, rewards);

        stakeInfo[_user].lastClaimDate = block.timestamp > stakeInfo[_user].finishPeriod ? stakeInfo[_user].finishPeriod : block.timestamp;

        emit Claim(_user, rewards);
    }

    function calculateReward(address _user) public isStakeExist(_user) view returns(uint256 rewards) {
        uint256 totalReward = amountReward;
        StakeInfo memory _stakeInfo = stakeInfo[_user];

        uint256 stakedAmount = _stakeInfo.amount;
        ...
        rewards = (stakedAmount * stakeDuration * totalReward / totalStaked / stakePeriod);
    }
